...
void GetBaverageInfo(const IBeverage& baverage)
{
	cout << baverage.GetDescription() << " costs " << baverage.GetCost() << endl;
}

void Task1()
{
	{
		auto standartLatte = make_unique<CLatte>(CoffeePortion::Standart);
		cout << standartLatte->GetDescription() << " costs " << standartLatte->GetCost() << endl;

		auto doubleLatte = make_unique<CLatte>(CoffeePortion::Double);
		cout << doubleLatte->GetDescription() << " costs " << doubleLatte->GetCost() << endl;

		auto standartCappuccino = make_unique<CCappuccino>(CoffeePortion::Standart);
		cout << standartCappuccino->GetDescription() << " costs " << standartCappuccino->GetCost() << endl;

		auto doubleCappuccino = make_unique<CCappuccino>(CoffeePortion::Double);
		cout << doubleCappuccino->GetDescription() << " costs " << doubleCappuccino->GetCost() << endl;
	}
	cout << endl;

	{
		...
		GetBaverageInfo(*blackTea);

		auto greenTea = make_unique<CGreenTea>()
			<< MakeCondiment<CCinnamon>();
		GetBaverageInfo(*greenTea);

		auto indianTea = make_unique<СIndianTea>()
			<< MakeCondiment<CIceCubes>(3, IceCubeType::Water);
		GetBaverageInfo(*indianTea);

		auto georgianTea = make_unique<СGeorgianTea>()
			<< MakeCondiment<CIceCubes>(1, IceCubeType::Dry);
		GetBaverageInfo(*georgianTea);
	}
	cout << endl;

	{
		auto littleMilkshake =
			make_unique<CMilkshake>(MilkshakePortion::Little);
		GetBaverageInfo(*littleMilkshake);

		auto averageMilkshake =
			make_unique<CMilkshake>(MilkshakePortion::Average);
		GetBaverageInfo(*averageMilkshake);

		auto bigMilkshake =
			make_unique<CMilkshake>(MilkshakePortion::Big);
		GetBaverageInfo(*bigMilkshake);
	}
	cout << endl;
}
...
void Task2()
{
	auto tea =
		...
		<< MakeCondiment<CChocolateSlice>(4)
		<< MakeCondiment<CLiquor>(LiquorType::Nutty)
