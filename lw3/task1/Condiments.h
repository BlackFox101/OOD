...
public:
		...
		: CCondimentDecorator(move(beverage))
	{
		if (quantity > 5)
		{
			throw std::invalid_argument("Maximum of 5 chocolate slices at a time");
		}
		m_quantity = quantity;
	}
...
protected:
	...
	std::string GetCondimentDescription()const override
	{
		return "ChocolateSlice x " + std::to_string(m_quantity);
	}
...
protected:
	...
	std::string GetCondimentDescription()const override
	{
		return std::string(m_liquorType == LiquorType::Nutty ? "Nutty" : "Chocolate")
			+ " liquor";
	}
